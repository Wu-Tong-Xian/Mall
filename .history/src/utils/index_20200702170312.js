...
export default {
  ...
  // 最近浏览模的块化
  recentlyViewed(item) {  
    // 判断本地有没有最近浏览的数组       this.$utils.recentlyViewed(整个点击商品的所有信息(index))
    // 比如存储fool 的所有最近浏览(跳转到详情就算)  this.$utils.recentlyViewed(this.floor1[index])
    if(!localStorage.recentlyViewed){
      let arr = []
      arr.push(item)
      localStorage.setItem('recentlyViewed',JSON.stringify(arr))
    }else{
      let arr = JSON.parse(localStorage.recentlyViewed)
      arr.push(item)
      localStorage.setItem('recentlyViewed',JSON.stringify(arr))
    }
},
