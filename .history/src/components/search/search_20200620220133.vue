      ...
      this.$api
        ...
        .then(res => {
          ...
          this.searchlist = res.data.list;
            ...
            let keyword = this.inputs   //获取输入框输入的内容
       let ob =   this.searchlist.map(item => {

            if (item.name.indexOf(keyword) !== -1) {  //判断这个字段中是否包含keyword
              //如果包含的话，就把这个字段中的那一部分进行替换成html字符
              //  val.replace(keyword, `<font color='#42cccc'>${keyword}</font>`)
