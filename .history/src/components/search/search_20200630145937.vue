...
<template>
  <div class="search-bg">
    ...
    <div v-if="inputs === ''">
      <div>
        <div class="flexbtw fl j-between">
          <div>搜索历史</div>
          <div v-if="SearchHistory.length >= 1" @click="delHistory">清除历史记录</div>
        </div>
        <van-cell v-if="SearchHistory.length < 1">暂无历史记录</van-cell>
        <div v-else class="history">
          <!-- 循环出所有的搜索历史 并且给搜索历史绑定方法  -->
    ...
    <div v-else>
      <!-- 循环显示商品信息 -->
      <van-cell v-for="item in searchlist" :key="item.id" @click="godetails(item.id)">
      ...
      setTimeout(() => {
        this.$api
          ...
          .then(res => {
            ...
            // 将搜索列表中关键字高亮设置
            this.searchlist.map(
              item => (item.name = keyWord(item.name, this.inputs))
            );
          })
          .catch(err => {
            console.log(err);
...
//  */
    const keyWord = (str, value) => {
      const replaceReg = new RegExp(this.inputs, "g");
      const replaceString = `<span style='color:red'>${this.inputs}</span>`;
