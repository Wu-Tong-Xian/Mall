<template>
  <div class="box1">
    ...
    <div v-if="inputs === ''" class="mg-l20 mg-t20">
      <!--  如果有历史记录 显示历史记录 -->
      <div class="fl j-between" v-if="searchHistory.length === 0">{{searchHistory.length}}
        <div class="fz14">历史记录</div>
        <div  class="fz14 mg-r20">删除历史记录</div>
      </div>

      <!-- 如果没有历史记录 -->
      <van-cell v-if="searchHistory.length === 0">暂无历史记录</van-cell>
      <div v-else >
...
export default {
  ...
  methods: {
    ...
    getHistory(val) {
      ...
      this.$router.push({ name: "detale", query: { ids: val } });
    },
