...
<div v-else class="t-center mg-t20">暂无收藏商品~~</div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    // 获取通过模块化方法（写在utils）存起来的收藏商品
    this.collect = JSON.parse(localStorage.getItem("collect"));
    // this.collect = uniqWith(this.collect, isEqual);  //用lodash uniqWith 方法做过滤 有bug 
    let hash = {};
                this.list = this.collect.reduce(function (item, next) {
                    hash[next.name] ? '' : hash[next.name] = true && item.push(next);
                    return item
                }, [])
                // 那个arr就是你要去重的数组，那个name就是你对象里面的，你可以根据这个name来去重，
