    ...
    <div v-else>
      ...
      <div v-else>
        ...
        <div class="fl j-between underline3">
          <div></div>
          <div>
            <van-button type="primary" size="small" @click="gosettlement">结算</van-button>
        ...
        <div v-for="(item,index) in shopList" :key="index" class="fl j-center">
          ...
          <div class="mg-l10">
            ...
            <div class="fl mg-t20 fz18 j-between">
              ...
              <!--vant 不进器-->
              <div>
                <!-- addOrSubtract 监听不进器的值发生改变(item.count 数量) -->
...
export default {
  ...
  data() {
    return {
      ...
      delarr: [], //盛放  商品id  因为做删除要 商品id(数组)  所以把商品id  push 进去
      shopList: [], //盛放  购物车数据
      value: 1, //不进器的默认值
      arr: [] // 过滤出来选中的数组 存放 结算时跳转到结算页面的 搜友商品id(数组)
    };
  },
  methods: {
    ...
    getCard() {
      this.$api
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            this.shopList = res.shopList; //搜索购物车数据
            // console.log(res);
          }
        })
        .catch(err => {
    ...
    gosettlement() {
      ...
      if (this.totalPrice === "0.00") {
        ...
        let idlist = []; //id列表 结算时需要 所有商品的id(数组)
        this.arr.map(item => {
        ...
        let obj = {
          ...
          count:this.totalPrice.count,
          list: this.arr
        };
        this.$store.state.paylists = obj; //购物车购买 的支付列表
        this.$store.state.buyway = 2; //购买方式 1:立即购买 2:购物车购买  默认 vuex里 1
        ...
        Dialog.confirm({
          ...
          .then(() => {
            ...
            this.shopList.map(item => {
              ...
              if (item.check === true) {
                //如果有选中项
                this.delarr.push(item.cid); // 后台要求 cid(数组) 所以把cid 放进delarr
                this.$api
                  .getDeleteShop(this.delarr)
                  .then(res => {
                    Toast.success("删除成功");
  ...
  computed: {
    ...
    totalPrice() {
      let sum = 0;
      if (this.shopList) { //公国filter过滤出选中的购物车商品
        this.arr = this.shopList.filter(item => {
          return item.check === true;
        });
        this.shopList.map(item => {  //计算总结
          //循环拿到每一项
          if (item.check === true) {
