  ...
  <div>
    ...
    <div>
      <van-tabs v-model="active">
        ...
        <van-tab title="用户评论">
          <div v-if="comment === ''" class="mg-tb20">暂无用户评论</div>
          <div v-else>
            <div class="fl j-between fz14">
              <div class="fl j-between">
                <!--用户头像-->
                <div>
                  <img :src="user.avatar" class="img2" alt />
                </div>
                <div class="mg-l10">
                  <!--用户名称-->
                  <div class="mg-tb10">{{nickname}}</div>
                  <!--用户评分-->
                  <div><van-rate v-model="comment.rate" readonly
                      allow-half  void-icon="star"  void-color="#eee" />
                  </div>
                </div>
              </div>
              <!--评论时间-->
            ...
            <!--评价内容-->
            <div class="mg-l20">评价内容:{{comment.content}}</div>
            <van-divider />
          </div>
    ...
    getGoods(id) {
      this.$api
        ...
        .then(res => {
          ...
          this.nickname = this.comment.user[0].nickname;
          this.avatar = this.comment.user[0].avatar;
