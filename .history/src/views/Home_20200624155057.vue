  ...
  <div>
    <div class="home-bg" v-if="falg !== 1">
      <div class="fl ceiling">
        ...
        <!--//一开始就显示的展示数据搜索-->
        <div>
          <!-- falg 父传子    send是子传父    左边是自定义方法名 右边有值-->
          <search1 :falg="falg" @send="send"></search1>
        </div>
      </div>
    ...
    <div v-else >
      <div class="fl ">
        ...
        <div class="bg-search">
          <!-- falg 父传子  send是子传父  左边是自定义方法名 右边是值-->
          <search :falg="falg" @send="send"></search>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      falg: -1, //用作搜索表示，传入子组件 点击子组件输入框或者搜索就把falg改变  并传递回来
      slides: [], //轮播图
      category: [], //分类商品
      advertesPicture: {},
      recommend: [], //商品推荐
