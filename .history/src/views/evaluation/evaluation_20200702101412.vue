<template>
  <div>
    ...
    <van-tabs v-model="active" class="evaluate">
      ...
      <!--已评价-->
      <van-tab title="已评价">
        <div v-for="item1 in completed" :key="item1.id">
          <div v-for="item2 in item1.goods" :key="item2.id">
            <van-card :title="item2.name" :thumb="item2.image_path">
              <template #footer>
                <van-button size="mini">
                  <van-button
                    @click="toEvaluationDetails(item2)"
                    round
                    plain
                    icon="chat"
                    size="small"
...
export default {
  ...
  methods: {
    ...
    // 评价详情
    toEvaluationDetails(item2){
      // 点击查看详情把点击这一项传到详情页
      localStorage.setItem('list',JSON.stringify('item2'))
